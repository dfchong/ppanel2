...
func getServers() *service.Group {
	...
	// CORS whitelist provider reads etc/cors.d/cors.yaml, mounted as a
	// directory from the ppanel-cors ConfigMap, and hot-reloads it in the
	// background. Directory mounts preserve the kubelet `..data` atomic
	// swap, so the mtime/size poller reliably picks up ConfigMap changes
	// (a subPath file mount would not, and would defeat hot-reload).
	corsProvider := middleware.NewCorsProvider("etc/cors.d/cors.yaml")
	ctx.CORS = corsProvider

	services := service.NewServiceGroup()
	services.Add(internal.NewService(ctx))
	services.Add(corsProvider)
