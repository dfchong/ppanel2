...
// CorsPolicy is implemented by middleware.CorsProvider. It is declared here
// (rather than importing the middleware package) to avoid a cyclic dependency:
// the middleware package already imports svc.
...
type ServiceContext struct {
	...
	// CORS holds the whitelist-backed CORS policy; nil keeps cross-origin
	// access closed (deny all) until a CorsProvider is wired in run.go.
	CORS         CorsPolicy
	Queue        *asynqx.Client
